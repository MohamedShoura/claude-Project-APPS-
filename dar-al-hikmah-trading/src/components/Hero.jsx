...
export default function Hero() {
  ...
  }, [displayed, deleting, wordIdx, T.typewriterWords])
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center overflow-hidden">
      <div className="absolute inset-0 z-0">
        ...
        {[...Array(12)].map((_, i) => (
          <div key={i} className="absolute rounded-full bg-yellow-500/20"
            style={{ width: `${Math.random() * 6 + 2}px`, height: `${Math.random() * 6 + 2}px`, left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%`, animation: `float ${4 + Math.random() * 4}s ease-in-out infinite`, animationDelay: `${Math.random() * 4}s` }} />
        ))}
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 py-32">
        <div className={`max-w-3xl ${isAr ? 'mr-0 ml-auto text-right' : ''}`}>
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border border-yellow-600/40 bg-yellow-950/30 backdrop-blur-sm mb-8 animate-fade-in-up ${isAr ? 'flex-row-reverse' : ''}`}>
            <span className="w-2 h-2 rounded-full bg-yellow-500 animate-pulse"></span>
            <span className="text-yellow-400 text-sm font-medium tracking-widest uppercase">{T.badge}</span>
          </div>
          ...
          <div className="text-2xl sm:text-3xl font-bold mb-6 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            <span className="text-gray-300">{T.typewriterPrefix}</span>
            <span className="text-yellow-400">{displayed}</span>
            <span className="cursor-blink text-yellow-400">|</span>
            <span className="text-gray-300">{T.typewriterSuffix}</span>
          </div>
          ...
          <div className={`flex flex-wrap gap-4 mb-14 animate-fade-in-up ${isAr ? 'flex-row-reverse' : ''}`} style={{ animationDelay: '0.4s' }}>
            <button onClick={() => scrollTo('quote')} className="btn-gold px-8 py-4 rounded-full text-base font-bold flex items-center gap-2 shadow-2xl">
          ...
          <div className={`flex flex-wrap gap-8 animate-fade-in-up ${isAr ? 'flex-row-reverse' : ''}`} style={{ animationDelay: '0.5s' }}>
            {[
              { value: '500+', label: T.stat1 },
              { value: '4', label: T.stat2 },
              { value: '10+', label: T.stat3 },
              { value: 'GCC', label: T.stat4 },
            ].map(stat => (
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-bounce">
        <div className="text-gray-500 text-xs uppercase tracking-widest">{T.scroll}</div>
